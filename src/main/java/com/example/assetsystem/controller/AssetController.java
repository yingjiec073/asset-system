...
public class AssetController {
    ...
    public ResponseEntity<ApiResponse<Asset>> create(@Valid @RequestBody Asset asset) {
        ...
        if (!saved) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.fail("新增资产失败"));
        }
    ...
    public ResponseEntity<ApiResponse<Asset>> update(@PathVariable Long id, @Valid @RequestBody Asset asset) {
        ...
        if (!updated) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.fail("更新资产失败"));
        }
    ...
    public ResponseEntity<ApiResponse<Void>> delete(@PathVariable Long id) {
        ...
        if (!removed) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.fail("资产不存在或已删除"));
        }
    ...
    public ResponseEntity<ApiResponse<AssetCategoryConfig>> updateCategory(@PathVariable Long id,
        ...
        if (categoryConfigService.getById(id) == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.fail("分类不存在"));
        }
        category.setId(id);
